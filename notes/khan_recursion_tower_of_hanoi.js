...
var hanoi = (function () {
    ...
    var Sprite = {
      ...
      gridMove: function (
        ...
        timeline.addEvent(
          workingFrame,
          duration,
          this.translate.bind(this, dx, dy)
        );
  ...
  var objects = (function () {
            ...
            if (this.peg.getTopDisk() === this) {
              for (var i = 0; i < pegs.length; i++) {
                ...
                if (
                  ...
                  if (
                    peg.countDisks() === 0 ||
                    this.number < peg.getTopDisk().number
                  ) {
                    this.moveToPeg(peg);
                  }
                  break;
    ...
    if (
      (numberOfDisks + 1) * DISK_HEIGHT >
      (WINDOW_SIZE / 6) * 3 - DISK_HEIGHT * 1.3
    ) {
      calcDiskHight =
        ((WINDOW_SIZE / 6) * 3 - DISK_HEIGHT * 1.3) / numberOfDisks;
    }
    ...
    for (var i = numberOfDisks - 1; i >= 0; i--) {
      ...
      disk.init({
        w:
          (((WINDOW_SIZE * 2) / 4 - WINDOW_SIZE / 4 - PEG_WIDTH * 1.3) /
            (1.3 * numberOfDisks)) *
            (i + 1) +
          PEG_WIDTH * 1.3,
        h: calcDiskHight,
    ...
    return {
      ...
      updateWorkingFrame: updateWorkingFrame,
      getWorkingFrame: getWorkingFrame,
    };
  })();
  ...
  var moveDisk = function (fromPeg, toPeg) {
    ...
    if (fromPeg === null) {
      ...
      validmove = false;
      badMoveMessage =
        "You attempted to move a disk from peg " + fromPeg.letter;
      badMoveMessage += " but that peg\nwas empty";
      badmove = true;
    } else if (
      toPeg.countDisks() > 0 &&
      fromPeg.getTopDisk().number > toPeg.getTopDisk().number
    ) {
      validmove = false;
      badMoveMessage =
        "You attempted to move disk " +
        fromPeg.getTopDisk().number +
        " from peg ";
      badMoveMessage +=
        fromPeg.letter + " onto disk " + toPeg.getTopDisk().number + " on peg ";
      badMoveMessage +=
        toPeg.letter +
    ...
    if (fromPeg !== null) {
      workingDisk = fromPeg.getTopDisk();
    }
    var numOnToPeg = null;
    if (toPeg !== null) {
      numOnToPeg = toPeg.disks.length;
    }

    objects.timeline.addEvent(
      nowFrame,
      duration,
      function () {
        ...
        objects.scene.draggingSprite =
          objects.scene.sprites.indexOf(workingDisk);

        var destY =
          toPeg.y + toPeg.h - (workingDisk.h + 0.1) * (numOnToPeg + 1);
      ...
      function () {
        ...
        workingDisk.y =
          toPeg.y + toPeg.h - (workingDisk.h + 0.1) * (disksUnder + 1);
      }
    );
  ...
  var isSolved = function (toPeg) {
    ...
    if (goalPeg.disks.length !== 5) {
      return false;
    }
    if (goalPeg === pegA) {
      return pegB.disks.length === 0 && pegC.disks.length === 0;
    }
    if (goalPeg === pegB) {
      return pegA.disks.length === 0 && pegC.disks.length === 0;
    }
    if (goalPeg === pegC) {
      return pegA.disks.length === 0 && pegB.disks.length === 0;
    }
  };
